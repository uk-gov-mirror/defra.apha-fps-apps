...
{
    /// <summary>
    /// DR-VAL-01's input (plan §3.2). Everything the shared validator needs, pre-fetched by
    ...
    /// §3.2). Lookups must be built via <see cref="BulkRatesValidationKeys"/> so key
    ...
    public sealed record ValidationContext
    {
        ...
        /// <summary>
        /// Gates the BC-05 live-vs-snapshot-absence check (plan §5.2) — a live positive AGRUP row,
        /// under a FEC TestCode being withdrawn, that was never part of the download snapshot.
        /// DR-API-09's own release-time check is deliberately scoped to snapshot-known rows only
        /// ("a live AGRUP row created after download... cannot be caught here — that gap is
        /// DR-WK-04's job"): a row invisible at download time is also invisible to whoever is
        ...
        /// Error-severity finding here can never surface where DR-API-09 says it shouldn't. The
        /// worker's revalidation pass (DR-WK-04, run inside the transaction against locked rows,
        /// §5.1) sets this true — that is exactly the point in the lifecycle where §5.2 requires
        /// this gap to be caught and the request failed.
        /// </summary>
