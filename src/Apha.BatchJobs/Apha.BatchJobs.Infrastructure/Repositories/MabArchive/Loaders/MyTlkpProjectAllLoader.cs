...
internal sealed class MyTlkpProjectAllLoader : MabArchiveExecutionLoaderBase
{
    internal MyTlkpProjectAllLoader(BatchJobsDbContext context) : base(context) { }
    ...
    protected override async Task<int> LoadCoreAsync(BatchJobsDbContext context, int year, CancellationToken cancellationToken)
    {
        ...
        var rows = sourceRows
            .Select(t => new MaDstMyTlkpProjectAll
            {
                ...
                WipCurrent = t.WipCurrent,
                ProjectStatus = t.ProjectStatus,
                DateCreated = t.DateCreated.HasValue
                    ? (DateTime?)(t.DateCreated.Value.Kind == DateTimeKind.Utc
                        ? t.DateCreated.Value
                        : DateTime.SpecifyKind(t.DateCreated.Value, DateTimeKind.Utc))
                    : null,
                FecCost = t.FecCost,
                Profit = t.Profit,
                BudgetCvl = t.BudgetCvl,
