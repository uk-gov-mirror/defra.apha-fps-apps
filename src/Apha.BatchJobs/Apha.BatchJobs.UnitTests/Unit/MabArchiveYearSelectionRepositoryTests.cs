...
public sealed class MabArchiveYearSelectionRepositoryTests
{
    ...
    public void Bucket_WhenOneOpenAndOnePlanned_ShouldSplitCorrectly()
    {
        ...
        var (openYears, plannedYears) = MabArchiveYearSelectionRepository.Bucket(rows);
    ...
    {
        var result = MabArchiveYearSelectionRepository.Validate(
            openYears: new[] { 2026 },
    ...
    public void Validate_WithNoOpenYear_ShouldThrow()
    {
        var ex = Assert.Throws<MabArchiveYearConfigurationException>(
            () => MabArchiveYearSelectionRepository.Validate(Array.Empty<int>(), Array.Empty<int>()));
    ...
    {
        var ex = Assert.Throws<MabArchiveYearConfigurationException>(
            () => MabArchiveYearSelectionRepository.Validate(new[] { 2025, 2026 }, Array.Empty<int>()));
    ...
    {
        var ex = Assert.Throws<MabArchiveYearConfigurationException>(
            () => MabArchiveYearSelectionRepository.Validate(new[] { 2026 }, new[] { 2027, 2028 }));
    ...
    {
        var ex = Assert.Throws<MabArchiveYearConfigurationException>(
            () => MabArchiveYearSelectionRepository.Validate(new[] { 2026 }, new[] { 2028 }));
