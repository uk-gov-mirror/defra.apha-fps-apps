...
CREATE OR REPLACE VIEW fps.vpostmort_vrd_split AS
 ...
 HAVING (((tlkptestcapability.planportfolio)::text = ANY (ARRAY[('TG0100'::citext)::text, ('PMPORT1'::citext)::text])) AND (monthlyoutput.month <= ( SELECT max(tblperiod.endperiod) AS month
           FROM fps.tblperiod
